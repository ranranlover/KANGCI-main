...
def load_causaltime_seq(data_path, device, scaler_type='minmax', n_max=None):
    ...
    if X.ndim == 3:
        ...
        # 1. 仅截取前 200 个样本
        # 此时 X 的形状变为 (200, 40, 40)
        # X = X[:200, :, :]
        # # 2. 获取新的形状参数
        # N_samples, T_per_sample, P_original = X.shape
        # T_total = N_samples * T_per_sample
        # print(f"Slicing top 200 samples. New shape: {X.shape}.")
        # print(f"Flattening to 2D shape ({T_total}, {P_original}).")
        # # 3. 合并维度 (200 * 40 = 8000)
        # # 结果形状为 (8000, 40)
        # X = X.reshape(T_total, P_original)
    elif X.ndim != 2:
...
def compute_batched_gc(model, full_input_seq, chunk_size, create_graph, lag_agg_local, cutoff_ratio=0.2):
    """
    分块计算 GCs，并求平均。
    """
    T = full_input_seq.shape[1]
    ...
    for start in range(0, T, chunk_size):
        ...
        #     smooth=smooth_flag,
        # )
        GCs_chunk = compute_gradient_gc_smooth_universal_v2(
        model,
        input_chunk,
        create_graph=create_graph,
        lag_agg_local=lag_agg_local,
...
def compute_batched_gc(model, full_input_seq, chunk_size, create_graph, lag_agg_local, cutoff_ratio=0.2):
    ...
    return accumulated_GCs / num_chunks
def compute_gradient_gc_smooth_universal_v2(model, input_seq_local, create_graph=False,
...
def infer_Grangercausalityv4_inge_plus_try_tosparse_1218(P, type, epoch, hidden_size, learning_rate,
    ...
    for i in range(epoch):
        ...
        if (i % gc_every) == 0:
            # 1. 计算原始 GCs (使用分块)
            # GCs_raw = compute_batched_gc(model_fwd, input_seq, GC_CHUNK_SIZE, gc_create_graph, lag_agg, cutoff_ratio)
            # GC2s_raw = compute_batched_gc(model_rev, reversed_input_seq, GC_CHUNK_SIZE, gc_create_graph, lag_agg,
            #                               cutoff_ratio)
        ...
        with torch.no_grad():
            ...
            score2,auprc2 = compute_roc_1219(GC, GC2s_np, False)
            # score_fusion = compute_roc_new_1211(GC, fused_np, True)
            score_fusion,auprc_fusion = compute_roc_1219(GC, fused_np, False)
...
def grid_search(param_grid):
    ...
    for params in param_list:
        ...
        avg_score = infer_Grangercausalityv4_inge_plus_try_tosparse_1211_single(36, 'pm25', 600,
                                                                                hidden_size=params['hidden_size'],
                                                                                learning_rate=params['learning_rate'],
                                                                                lambda_gc_sparse_base=params[
                                                                                    'lambda_gc_sparse_base'],
                                                                                cutoff_ratio=params['cutoff_ratio'],
                                                                                tau=params['tau'],beta_prior=params['beta_prior'])
...
if __name__ == '__main__':
    ...
    #
    # }
    # zhege haoxiang softmax no good
    # 0.88796  0.7330  256   0.001  0.008  0.6  seem 0.8 good a bit       new  0.90575   0.76208
    param_grid = {
        'hidden_size': [128],  ##128   orgin256
        'learning_rate': [0.001,0.0008],  # 0.005 0.001
        'lambda_gc_sparse_base': [0.008,0.01,0.007],  #
        'cutoff_ratio': [0.8],
        'lag_agg': ['softmax'],
        'tau': [0.1],
        'tau_prior': [0.03,0.04],
 ...
 # {'cutoff_ratio': 0.8, 'data_path': '/home/user/wcj/KANGCI-main/realdataset', 'hidden_size': 256, 'lag_agg': 'mean', 'lambda_gc_sparse_base': 0.008, 'learning_rate': 0.001, 'tau': 0.1}
# {'cutoff_ratio': 0.8, 'data_path': '/home/user/wcj/KANGCI-main/realdataset', 'hidden_size': 256, 'lag_agg': 'mean', 'lambda_gc_sparse_base': 0.009, 'learning_rate': 0.001, 'tau': 0.1}


# Training with params: {'beta_prior': 15, 'cutoff_ratio': 0.8, 'hidden_size': 128, 'lag_agg': 'softmax', 'lambda_gc_sparse_base': 0.008, 'learning_rate': 0.001, 'tau': 0.1, 'tau_prior': 0.03}

# Best params: {'beta_prior': 20, 'cutoff_ratio': 0.8, 'hidden_size': 256, 'lag_agg': 'softmax', 'lambda_gc_sparse_base': 0.008, 'learning_rate': 0.001, 'tau': 0.1, 'tau_prior': 0.03} with avg score: 0.928490889680569
# Best params: {'beta_prior': 15, 'cutoff_ratio': 0.8, 'hidden_size': 128, 'lag_agg': 'softmax', 'lambda_gc_sparse_base': 0.008, 'learning_rate': 0.001, 'tau': 0.1, 'tau_prior': 0.03} with avg score: 0.7905865851744749
